...
Fc_dtheta = 2.0 # cut-off frequency of the filter in Hz
...
if __name__ == "__main__":
    ...
    filename = 'balance_test_%i' % trial_num
...
>>>>>>> f678940d8d14b597afb673589c533e04bb1b0d2c
        ...
        theta_err_x = theta_x - 0.002 # goal value of 0, positive bias causes -y direction movement
        theta_err_y = theta_y - 0.001 # goal value of 0, positive bias causes -x direction movement
        ...
        if -0.003 < theta_err_x < 0.003:
            i_theta_err_x = 0.0
        if -0.003 < theta_err_y < 0.003:
            i_theta_err_y = 0.0

        # TORQUE COMPUTATION
        # compute xyz torques from theta PID and BT controller and convert to 123 torques
        Tx = KP_THETA_X * theta_err_x + KD_THETA_X * d_theta_err_x + KI_THETA_X * i_theta_err_x + Tx_dphi #- bt_controller.Ly * 0.8
        Ty = KP_THETA_Y * theta_err_y + KD_THETA_Y * d_theta_err_y + KI_THETA_Y * i_theta_err_y + Ty_dphi #+ bt_controller.Lx * 0.8 
        ...
        print("THETA X: {:.2f}, THETA Y: {:.2f}".format(theta_x, theta_y))
